...
if ($_POST['submit_bt'] == 'บันทึกข้อมูล' || $_POST['submit_bt'] == 'บันทึกข้อมูล และแก้ไขต่อ'){
		...
		if($contact_map->Save()){		
			...
			header('location:' . ADDRESS_ADMIN_CONTROL . 'google_map&action=edit&id=' . $contact_map->GetPrimary());
			die();
		}else{
...
<div class="da-panel-header"> <span class="da-panel-title"> <i class="icol-<?php echo ($contact_map->GetPrimary() != '') ? 'map' : 'add'?>"></i> <?php echo ($contact_map->GetPrimary() != '') ? '' : ''?> GOOGLE MAP </span> </div>
...
<div class="da-form-inline">
    ...
    <div class="da-form-row">
        <label class="da-form-label">PRE VIEW</label>
        <div class="da-form-item large">
            <?php echo ($contact_map->GetPrimary() != '') ? $contact_map->GetValue('google_map') : ''; ?>
        </div>
    </div>
</div>
<div class="btn-row">
